...
class Template_
{
    ...
    function print_($fid, $scope = '', $sub = false)
    {
        ...
        if ($file_type === 'tpl') {
            ...
            if ($sub) {
                ...
                if ($this->notice) {
                    error_reporting($this->_php_error_reporting | E_NOTICE | E_WARNING);
    ...
    function _empty_out_cache_dir($dir_path)
    {
        ...
        if ($this->_shell_exec_available()) {
            substr(__file__, 0, 1) === '/' ? @shell_exec('rm -rf "' . $dir_path . '/"*') : @shell_exec('del "' . str_replace('/', '\\', $dir_path) . '\\*" /s/q');
        } else {
...
// Below methods are deprecated.
    ...
    {
        if ($n === 1)
            $this->b1 = &$this->var_[$this->_current_scope][$id];
        else
            $this->{'b' . $n} = &$this->{'b' . --$n}[count($this->{'b' . $n}) - 1][$id];
    }
    ...
    {
        if ($n === 1)
            $this->var_[$this->_current_scope][$id] = is_array($arg) ? $arg : array_pad(array(), $arg, 1);
        else
            $this->{'b' . --$n}[count($this->{'b' . $n}) - 1][$id] = is_array($arg) ? $arg : array_pad(array(), $arg, 1);
    }
...
function _template_notice_handler($type, $msg, $file, $line)
{
    ...
    switch ($type) {
        case E_NOTICE :$msg = '<span style="font:12px tahoma,arial;color:green;background:white">Template_ Notice #1: ' . $msg . '</span>';
            break;
        case E_WARNING :
        case E_USER_WARNING:$msg = '<b>Warning</b>: ' . $msg;
            break;
        case E_USER_NOTICE :$msg = '<b>Notice</b>: ' . $msg;
            break;
        case E_USER_ERROR :$msg = '<b>Fatal</b>: ' . $msg;
            break;
        default :$msg = '<b>Unknown</b>: ' . $msg;
            break;
    }
    echo "<br />\n" . $msg . "<br />\n";
}
