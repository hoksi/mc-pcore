...
class Template_Compiler_
{
    ...
    function _compile_template($tpl, $template_path, $compile_base, $compile_header)
    {
        ...
        $this->all_functions = array_merge(
            $functions['internal'], $functions['user'], array('isset', 'empty', 'eval', 'list', 'array', 'include', 'require', 'include_once', 'require_once')
        );
        ...
        while ($plugin_file = $d->read()) {
            ...
            if ($match[1] === 'object') {
                ...
                switch ($match[1]) {
                    case 'function' : $this->func_plugins[$match[2]] = $plugin;
                        break;
                    case 'prefilter' : $this->prefilters[$match[2]] = $plugin;
                        break;
                    case 'postfilter': $this->postfilters[$match[2]] = $plugin;
                        break;
                }
        ...
        // apply prefilter
        if (trim($this->prefilter))
            $source = $this->_filter($source, 'pre');
        ...
        for ($this->mark_php = 0, $mark_tpl = 0, $this->_index = 0, $s = count($this->_split); $this->_index < $s; $this->_index++) {
            ...
            switch (strtolower($this->_split[$this->_index])) {
                ...
                case '}-->':
                case '}' :
                    if ($mark_tpl !== $this->_index - 2)
                        break;
                    if (!$result = $this->_compile_statement($this->_split[$this->_index - 1]))
                        break;
                    if (is_array($result)) {
                        ...
                        if ($this->mark_php) {
                            ...
                            if ($result[0] & 6)
                                $this->_split[$mark_tpl - 1] = preg_replace('/(\r\n|\n|\r)[ \t]+$/', '$1', $this->_split[$mark_tpl - 1]);
                        ...
                        if ($result === -2)
                            $this->report('Error #7', 'unexpected directive "<b>/</b>"', true, true);
                        elseif ($result === -3)
                            $this->report('Error #8', 'unexpected directive "<b>:</b>"', true, true);
                        if (!empty($erlist))
                            foreach ($erlist as $er)
                                $this->report('Warning #2', '<b>{' . $er[0] . '}</b> may be syntax error', true, $er[1]);
                        $this->exit_();
                    }
            }
        }
    ...
    function _save_result($compile_path, $compile_header, $init_code, $source)
    {
        ...
        $fp_cpl = fopen(str_replace('//', '/', $compile_path), 'wb');
        if (false === $fp_cpl) {
        ...
        if (!$this->on_ms)
            @chmod($compile_path, $this->permission & ~0111);
    }
    ...
    function _compile_statement($statement)
    {
        ...
        switch ($match[2]) {
            ...
            case '@' :
                ...
                if ($d === 1 && $this->in_div)
                    $this->_size_info[$this->in_div][$src] = 1;
                $this->_loop_stack[] = $src;
                $this->_loop_info[$src] = $d;
                return array(2, 'if($TPL_' . $src . '_' . $d . '){');
            case '?' :
                if (($stt = $this->_compile_branch($src, 0, 0)) === 0)
                    return -1;
                $this->_control_stack[] = substr($stt, 0, 2) === 'if' ? '?' : '$';
                return array(2, $stt);
            case ':' :
                if (strlen($src)) {
                    if (($stt = $this->_compile_branch($src, 0, 1)) === 0)
                        return 0;
                    if (empty($this->_control_stack))
                        return -3;
                    switch (array_pop($this->_control_stack)) {
                        case '?':
                            if (($xpr = $this->_compile_expression($src, 0, 0)) === 0)
                                return 0;
                            $this->_control_stack[] = '?';
    ...
    function _compile_branch($source, $escape = 0, $case = 0)
    {
        ...
        for ($i = 0; $i < count($split); $i += 2) {
            if (($case_pos = strpos($split[$i], ':')) !== false)
                break;
        }
        if ($case_pos !== false) {
    ...
    function _compile_expression($expression, $escape = 0, $no_directive = 0)
    {
        ...
        for ($xpr = '', $i = 0; strlen($expression); $expression = substr($expression, strlen($m[0])), $i++) { // 
            ...
            if (!empty($m[10])) { // (.+)
                ...
                if ($prev_is_operand || $var_state[0])
                    return 0;
                $prev_is_operand = 1;
                    ...
                    case '(' :
                        if ($var_state[0]) {
                            ...
                            if (in_array($func, $this->func_plugins)) {
                                ...
                                if ($this->safe_mode)
                                    in_array($func, $this->safe_mode_functions) or $this->exp_error[] = array(5, $m[2]);
                                else
                                    in_array($func, $this->all_functions) or $this->exp_error[] = array(7, $m[2]);
                            }
                        }
                        $prev_is_operand = 0;
                        $prev_is_func = 1;
                ...
                if ($var_state[0] || !$prev_is_operand || empty($par_stack) || array_pop($par_stack) !== '[')
                    return 0;
                if (empty($m[5]))
                    $m[5] = '';
                switch ($m[5]) {
                ...
                if ($prev_is_operand || preg_match('/ [+\-]$/', $xpr))
                    return 0;
                ...
                switch ($m[8]) {
                    ...
                    case'--':
                        return 0;
                    case ',':
                        if (!$prev_is_operand || empty($par_stack) || $par_stack[count($par_stack) - 1] !== 'f')
                            return 0;
                        $prev_is_operand = 0;
                        break;
                    case '(':
                        ...
                        break;
                    case ')':
                        if (!$prev_is_operand && !$prev_is_func || empty($par_stack) || array_pop($par_stack) === '[')
                            return 0;
                        $prev_is_operand = 1;
                        break;
                    case '!':
        ...
        if (!empty($par_stack) || !$prev_is_operand || $var_state[0] || $no_directive && $i === 1 && $number_used)
            return 0;
        if ($escape)
            return 1;
        ...
        foreach ($this->_outer_size as $loop_id => $depth) {
            if ($depth === 1 && $this->in_div)
                $this->_size_info[$this->in_div][$loop_id] = 1;
        ...
        if ($func_list) {
            ...
            if ($this->in_div)
                $this->_set_function($func_list, $this->in_div);
        }
        if ($this->exp_object) {
            $this->_set_class($this->exp_object);
            if ($this->in_div)
                $this->_set_class($this->exp_object, $this->in_div);
        }
        return $xpr;
    }
    ...
    function _compile_array($subject, $end = '')
    {
        if (preg_match('/^\.+/', $subject, $match)) { // ..loop
            ...
            if ($D = strpos($subject, '.')) { // id.var
                ...
                if ($id === 'p' || $id === 'P') { // p.object
                    ...
                    if ($end !== 'stop')
                        $this->exp_error[] = array(2, $subject);
                    return $var;
                ...
                $var = '';
                $el = '';
                if (in_array($id, $this->_loop_stack))
                    $depth = $this->_loop_info[$id];
            }
            if (empty($depth)) { // not loop
                if ($id[0] === '_') {
                    ...
                    if (in_array($id, $this->auto_globals))
                        return '$' . $id . $el;
        ...
        switch ($var) {
            case 'key_':
                if ($end !== 'stop')
                    $this->exp_error[] = array(-1, $subject);
                elseif (isset($this->exp_loopvar[$depth]))
                    $this->exp_loopvar[$depth] |= 1;
                else
                    $this->exp_loopvar[$depth] = 1;
                return '$TPL_K' . $depth;
            case 'value_':
                if (isset($this->exp_loopvar[$depth]))
                    $this->exp_loopvar[$depth] |= 2;
                else
                    $this->exp_loopvar[$depth] = 2;
                return '$TPL_V' . $depth;
            case 'index_':
    ...
    function _set_function($func_list, $divname = '')
    {
        ...
        foreach ($func_list as $func => $line) {
            if (!in_array($func, $prev_list))
                $this->func_list[$divname][$func] = $line;
        }
    }
    ...
    function _set_class($obj_list, $divname = '')
    {
        ...
        foreach ($obj_list as $obj => $line) {
            if (!in_array($obj, $prev_list))
                $this->obj_list[$divname][$obj] = $line;
        }
    }
    ...
    function report($type, $msg, $file = false, $line = false)
    {
        ...
        if ($file)
            $report .= ' in <b>' . (is_string($file) ? $file : $this->tpl_path) . '</b>';
        if ($line) {
            $line = is_int($line) ? $line : $this->nl_cnt;
            foreach ($this->nl_del as $key => $val)
                if ($key <= $line)
                    break;
